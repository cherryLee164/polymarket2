...
const PROFIT_BALANCE_SNAPSHOTS_PATH = path.join(DATA_DIR, "profit-balance-snapshots.jsonl");
const BALANCE_ADJUSTMENTS_PATH = path.join(DATA_DIR, "balance-adjustments.jsonl");
...
const DOMESTIC_CITY_SLUGS = new Set(
  WEATHER_CITY_CONFIGS.filter((item) => item.region === "domestic").map((item) => item.citySlug),
);
...
// 模拟下单 NO 价格上限（与实盘 WEATHER_LIVE_MAX_NO_PRICE 一致，默认0.90）
const OFFSET_SIM_MAX_NO_PRICE = normalizeProbabilityThreshold(
  process.env.WEATHER_LIVE_MAX_NO_PRICE ?? process.env.WEATHER_SIM_MAX_NO_PRICE ?? 0.90,
  0.90,
);
...
function buildWeatherOffsetSimulation(records, todayYmd, liveConfig = null) {
  ...
  for (const record of records || []) {
    ...
    for (const candidate of candidates) {
      ...
      // 与实盘一致：NO 价格超过上限不下单（默认0.90）
      if (buyNoPrice > OFFSET_SIM_MAX_NO_PRICE) {
        continue;
      }
...
function buildFollowYesterdaySimulation(records, todayYmd) {
  ...
  for (const record of records || []) {
    ...
    // 与实盘一致：NO 价格超过上限不下单（默认0.90）
    if (buyNoPrice > OFFSET_SIM_MAX_NO_PRICE) continue;
...
// 读取充值/提现调整记录，返回 Map<date, netAdjustment>
async function readBalanceAdjustments() {
  try {
    const text = await fs.readFile(BALANCE_ADJUSTMENTS_PATH, "utf8");
    const lines = text.split(/\r?\n/).filter(Boolean);
    const byDate = new Map();
    for (const line of lines) {
      try {
        const item = JSON.parse(line);
        if (!item || !item.date || !Number.isFinite(Number(item.amount))) continue;
        const date = String(item.date);
        const prev = byDate.get(date) || 0;
        byDate.set(date, round(prev + Number(item.amount), 6));
      } catch {
        continue;
      }
    }
...
// 写入一条充值/提现调整记录
export async function addBalanceAdjustment(amount, note = "") {
  await ensureDir(DATA_DIR);
  const now = new Date();
  const beijingDate = new Date(now.getTime() + 8 * 3600 * 1000).toISOString().slice(0, 10);
  const record = {
    date: beijingDate,
    amount: round(Number(amount), 6),
    note: note || (Number(amount) >= 0 ? "充值" : "提现"),
    createdAt: now.toISOString(),
  };
  await fs.appendFile(BALANCE_ADJUSTMENTS_PATH, JSON.stringify(record) + "\n", "utf8");
  return record;
}

async function readProfitBalanceSnapshots() {
  try {
    const text = await fs.readFile(PROFIT_BALANCE_SNAPSHOTS_PATH, "utf8");
    const lines = text.split(/\r?\n/).filter(Boolean);
    // 同一天可能有多个记录（pending/confirmed），优先取 confirmed，否则取最后一条
    const byDate = new Map();
    for (const line of lines) {
      try {
        const item = JSON.parse(line);
    ...
    const sorted = [...byDate.values()]
      .map((item) => ({
        ...
        amount: round(Number(item.amount), 6),
        status: item.status || "pending",
        source: item.source || null,
        capturedAt: item.capturedAt || null,
      }))
      .sort((a, b) => String(a.date).localeCompare(String(b.date)));
    // 计算每日收益 = 当天余额 - 前一天余额 - 当日净调整额（充值不算收益，提现不算亏损）
    const adjustments = await readBalanceAdjustments();
    const withPnl = sorted.map((item, idx) => {
      const prev = idx > 0 ? sorted[idx - 1] : null;
      const adj = adjustments.get(item.date) || 0;
      const pnl = prev && Number.isFinite(prev.amount) ? round(item.amount - prev.amount - adj, 6) : null;
      return { ...item, pnlUsd: pnl, adjustmentUsd: adj || null };
    });
    // 按月分组
    ...
    const months = [...byMonth.keys()].sort((a, b) => String(b).localeCompare(String(a)));
    const monthlyGroups = months.map((month) => {
      const items = byMonth.get(month);
      const totalPnl = items.reduce((sum, item) => sum + (Number.isFinite(item.pnlUsd) ? item.pnlUsd : 0), 0);
      const startBalance = items[0]?.amount ?? null;
      const endBalance = items[items.length - 1]?.amount ?? null;
      return {
        month,
        items: items.sort((a, b) => String(b.date).localeCompare(String(a.date))),
        totalPnlUsd: round(totalPnl, 6),
    ...
    return {
      ...
      latestBalance: withPnl[withPnl.length - 1]?.amount ?? null,
      latestDate: withPnl[withPnl.length - 1]?.date ?? null,
      totalRecords: withPnl.length,
    };
  } catch {
    return {
      months: [],
...
export async function getWeatherDashboardSnapshot(options = {}) {
  ...
  // 0度策略和跟昨天偏移策略：从 records.json 实时生成，避免依赖历史残留的 sim-orders.json
  // 只统计国内城市，0度策略只取 offset=0 的记录
  const baseRecords = baseSnapshot.records || [];
  const zeroOffsetSim = buildWeatherOffsetSimulation(baseRecords, baseSnapshot.localDate, liveConfig);
  const followYesterdaySim = buildFollowYesterdaySimulation(baseRecords, baseSnapshot.localDate);
  const zeroOffsetRecords = (zeroOffsetSim.records || []).filter(
    (r) => r.temperatureOffsetC === 0 && DOMESTIC_CITY_SLUGS.has(r.citySlug),
  );
  const followYesterdayRecords = (followYesterdaySim.records || []).filter(
    (r) => DOMESTIC_CITY_SLUGS.has(r.citySlug),
  );
  simOrders.zeroOffset = {
    id: zeroOffsetSim.id,
    label: "0 度策略",
    records: zeroOffsetRecords,
    summary: summarizePerformance(zeroOffsetRecords, baseSnapshot.localDate, {
      pnlField: "accountingPnlUsd",
      stakeField: "stakeUsd",
    }),
  };
  simOrders.followYesterday = {
    id: followYesterdaySim.id,
    label: "跟昨天偏移",
    records: followYesterdayRecords,
    summary: summarizePerformance(followYesterdayRecords, baseSnapshot.localDate, {
      pnlField: "accountingPnlUsd",
      stakeField: "stakeUsd",
    }),
  };
  const profitSnapshots = await readProfitBalanceSnapshots();
  return {
    ...baseSnapshot,
