...
function buildFollowYesterdaySimulation(records, todayYmd) {
  ...
  for (const record of records || []) {
    if (!record.citySlug || !record.date) continue;
    const actual = toReasonableTemperature(record.actualMaxTempC);
    const forecast = toReasonableTemperature(record.forecastMaxTempC);
    const delta =
  ...
  for (const record of records || []) {
    ...
    const targetTempC = matchedCandidate
      ? toReasonableTemperature(matchedCandidate.targetTempC ?? Number(record.forecastMaxTempC) + followOffset)
      : toReasonableTemperature(Number(record.forecastMaxTempC) + followOffset);
    const marketSlug = matchedCandidate?.marketSlug || record.marketSlug;
    const marketBucketValue = matchedCandidate
      ? toReasonableTemperature(matchedCandidate.marketBucketValue ?? matchedCandidate.targetTempC)
      : targetTempC;
