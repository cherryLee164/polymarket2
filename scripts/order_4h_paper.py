...
WINDOW_MINUTES = 240
EVENT_PREFIX = "btc-updown-4h-"
EVENT_SUFFIX = ""
PRICE_SIDE = "BUY"
SAMPLE_INTERVAL_MS = int(paper.get_first_env(["ORDER_4H_PAPER_SAMPLE_INTERVAL_MS"], "30000"))
USD_PER_LEG = float(paper.get_first_env(["ORDER_4H_PAPER_USD_PER_LEG"], "1"))
SETTLEMENT_DELAY_MINUTES = int(
    paper.get_first_env(["ORDER_4H_PAPER_SETTLEMENT_DELAY_MINUTES"], "60")
)
MAX_SAMPLES_DEFAULT = int(paper.get_first_env(["ORDER_4H_PAPER_MAX_SAMPLES"], "0"))
...
def build_4h_slug(target: datetime) -> str:
...
def build_event_state(meta: Dict[str, Any], session_label: str) -> Dict[str, Any]:
    ...
    resolve_after = event_end + timedelta(minutes=SETTLEMENT_DELAY_MINUTES)
    return {
        ...
        "settlementResolveAfterAt": resolve_after.isoformat(),
...
def build_session_summary(session_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "sampleIntervalMs": SAMPLE_INTERVAL_MS,
        "usdPerLeg": USD_PER_LEG,
        "settlementDelayMinutes": SETTLEMENT_DELAY_MINUTES,
...
def log_rankings(session_state: Dict[str, Any]) -> None:
    ...
    paper.log("4H paper ranking | " + " | ".join(top_lines))
...
def resolve_event_if_ready(
    ...
    if event_state.get("settlement", {}).get("status") == "resolved":
        return False
    resolve_after = paper.parse_date(event_state.get("settlementResolveAfterAt"))
    if resolve_after is not None and datetime.now(paper.UTC) < resolve_after:
        return False
    ...
    write_session_files(session_paths, session_state)
    paper.log(
        f"Resolved {event_state['slug']} | winner={winner_side} | "
        + ", ".join(
...
def sample_event(session_state: Dict[str, Any], session_paths: Dict[str, Path], event_state: Dict[str, Any]) -> None:
    ...
    write_session_files(session_paths, session_state)
    paper.log(
        f"4H Sample {event_state['sampleCount']} | Up {prices['upCents']:.3f}c "
        f"Down {prices['downCents']:.3f}c | waiting={waiting_first} active={active_positions} "
        f"skipped={skipped} resolved={resolved}"
    )


def build_session_paths(session_label: str) -> Dict[str, Path]:
    session_dir = ROOT_DIR / "data" / "orders" / "paper-4h" / session_label
...
def parse_args(argv: List[str]):
    parser = argparse.ArgumentParser(description="Standalone 4H paper strategy runner")
    parser.add_argument("--session-label", default="", help="Optional explicit session label")
    parser.add_argument(
        "--fresh-session",
        action="store_true",
        help="Start a fresh 4H paper session instead of continuing the rolling cumulative session",
    )
    parser.add_argument(
        "--max-samples",
        type=int,
...
def main(argv: List[str]) -> int:
    ...
    if args.session_label:
        ...
        session_label = f"paper-4h-{paper.format_filename(process_started_at)}"
    else:
    ...
    paper.log(
        f"4H paper worker ready | session={session_label} sample={round(SAMPLE_INTERVAL_MS / 1000, 1)}s "
        f"usdPerLeg=${USD_PER_LEG} settleDelay={SETTLEMENT_DELAY_MINUTES}m "
        f"groups={','.join(strategy['label'] for strategy in STRATEGY_GROUPS)}"
    )
    paper.log(f"4H paper data dir: {session_paths['root']}")
    ...
    while True:
        ...
        try:
            ...
            paper.log(f"4H paper event lookup failed: {exc}")
        ...
        if event_state is None and meta["eventStart"] < process_started_at:
            if not skipped_partial_window_logged:
                paper.log(
                    f"Current 4H window {meta['slug']} started at {meta['eventStart'].isoformat()} "
                    "before this process started. Waiting for the next full 4H window."
                )
        ...
        if event_state is None:
            ...
            skipped_partial_window_logged = False
            paper.log(
                f"Started 4H paper event {meta['slug']} | "
                f"{meta['eventStart'].astimezone(paper.LOG_TZ).strftime('%m-%d %H:%M:%S')} -> "
                f"{meta['eventEnd'].astimezone(paper.LOG_TZ).strftime('%m-%d %H:%M:%S')}"
            )
        else:
            active_hour_key = hour_key
        ...
        try:
            ...
            paper.log(f"4H paper sample failed for {event_state['slug']}: {exc}")
        ...
        for pending_hour_key, pending_state in list((session_state.get("events") or {}).items()):
            ...
            try:
                ...
                paper.log(f"4H paper resolve failed for {pending_state['slug']}: {exc}")

        try:
            resolve_event_if_ready(session_state, session_paths, event_state)
        except Exception as exc:
            paper.log(f"4H paper resolve failed for {event_state['slug']}: {exc}")

        if args.max_samples > 0 and sample_counter >= args.max_samples:
            paper.log(f"4H paper reached max samples={args.max_samples}; exiting.")
            return 0
